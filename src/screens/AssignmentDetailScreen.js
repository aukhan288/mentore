...
import { View, Text, TextInput, Image, TouchableOpacity, Modal, Alert, PermissionsAndroid, Pressable, ScrollView, FlatList, Dimensions, StyleSheet } from "react-native";
...
import { attachmentUpload, submitOrder, getAssignment, BASE_URL } from '../services/userServices';
...
const AssignmentDetail = (props) => {
  ...
  useEffect(() => {
    setLoader(true)
    getAssignment(props?.route?.params?.assignmentId)
    .then(res=>{
        setLoader(false)
        // if(res?.success){
...
const downloadAssignment = async (item) => {
    setLoader(true)
  ...
  const fileExtension = item?.completed_file.split('.').pop();
  const fileName = `${assignment?.assignments_id}.${fileExtension}`;
  ...
  const configfb = {
      ...
      addAndroidDownloads: {
          ...
          mediaScannable: true,
          title: fileName,
          path: `${RNFS.DownloadDirectoryPath}/${fileName}`, // Ensure this is where you want it
      },
      path: `${dirToSave}/${fileName}`, // Ensure correct path for both platforms
  };
      ...
      RNFetchBlob.config(configOptions || {})
        .fetch('GET', `${BASE_URL}public/${item?.completed_file}`, {})
        .then(res => {
          setLoader(false)
          if (Platform.OS === 'ios') {
...
const downloadAssignmentRevision = async (item,index) => {
  setLoader(true)
    const { dirs } = RNFetchBlob.fs;
...
const fileExtension = item?.completed_file.split('.').pop();
const fileName = `${assignment?.assignments_id}-Revision-${index+1}.${fileExtension}`;
    ...
    return (
        <View key={revision.id} style={{borderWidth: 1, borderColor: '#e2e2e2', borderRadius: 5, padding: 5, marginBottom: 10}}>
            ...
            {revision?.attachments?.length > 0 ? (
             ...
             {revision?.is_completed!='0' && <Pressable
        onPress={()=>{downloadAssignmentRevision(revision,index)}}
        style={{backgroundColor:'#031D53',overflow:'hidden', borderRadius:8,marginTop:15}}>
        <Text style={{color:'#FFF',textAlign:'center', paddingVertical:10}}>Download Now</Text>
      </Pressable> }
           <View>
